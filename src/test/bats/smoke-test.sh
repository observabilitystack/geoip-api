...
@test "[meta] Prometheus metrics ok" {
  curl -fs "http://localhost:8080/actuator/prometheus"
}

@test "[meta] Prometheus metrics available" {
  curl -fs "http://localhost:8080/actuator/prometheus"|grep "geoip_database"
}

@test "[geoip] 8.8.8.8" {
  run curl -fs "http://localhost:8080/8.8.8.8"
  assert_success
...
@test "[geoip] 149.233.213.224" {
  run curl -fs "http://localhost:8080/149.233.213.224"
  assert_success
